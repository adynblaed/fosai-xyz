...
Prompting generative pretrained transformers (GPTs) for effective assistance can be difficult. While 'prompt engineer' is a bit misleading in its title, there is real value in structuring the way you ask and communicate for ideas, text, or conversation when interacting with AI & LLMs. If you are using ChatGPT, you can leverage some of the techniques below with the power of the `custom instructions` option to empower your chat sessions with even more specific context built around your project or chat. These concepts can apply for any `system message` or prompt injection/chain technique.
